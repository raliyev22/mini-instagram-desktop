...
public class DiscoverPage extends JFrame {
     ...
     */
    public DiscoverPage(String nickname, String password, String name, String surname, String age, String email, String type) {
...
//logging info
				info.log(String.format("%s moved to Profile Page",DiscoverPage.this.nickname));
            	NormalProfilePageWithHierarchies frame = new NormalProfilePageWithHierarchies(DiscoverPage.this.nickname, DiscoverPage.this.password, DiscoverPage.this.name, DiscoverPage.this.surname, DiscoverPage.this.age, DiscoverPage.this.email, DiscoverPage.this.type);
                dispose();
            }
        });
        ...
        searchButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (SignUpMethods.existenceOfNickname(searchNickname)) {
                    ...
                    if (user.get(0).equals(DiscoverPage.this.nickname)) {
                        new NormalProfilePageWithHierarchies(user.get(0), user.get(1), user.get(2), user.get(3), user.get(4), user.get(5), user.get(6));
                        dispose();
                    } else {
                        new ProfilePagePublicly(user.get(0), user.get(1), user.get(2), user.get(3), user.get(4), user.get(5), user.get(6), DiscoverPage.this.nickname);
                        dispose();
                    }
                } 
                    ...
                    JOptionPane.showMessageDialog(DiscoverPage.this, "This username does not exist.", "Username Not Found", JOptionPane.ERROR_MESSAGE);
                }
            }
        });
     ...
     * @param nickname
     * @param path
     * this method create nameLabel,imageLabel,like, dislike, no reaction button for each image in panel and add this panel to main pane;
     */
...
//logging info
                		info.log(String.format("%s moved to look at %s'Profile Page",DiscoverPage.this.nickname,user.get(0)));
                        new ProfilePagePublicly(user.get(0), user.get(1), user.get(2), user.get(3), user.get(4), user.get(5), user.get(6), DiscoverPage.this.nickname);
                        dispose();
                    }
                }
			...
			likeButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					try {
						...
						if (hasUserLikedImage(nickname,path,DiscoverPage.this.nickname)==false && hasUserDislikedImage(nickname,path,DiscoverPage.this.nickname)==false) {
							...
							currentLikes += 1;
//logging info
	                		info.log(String.format("%s liked %s's photo ",DiscoverPage.this.nickname,nickname));
							...
							writeLikedInfo(nickname, path, DiscoverPage.this.nickname);
							deleteDislikeInfo(nickname, path, DiscoverPage.this.nickname);
	
						
						}
						
						else if (hasUserLikedImage(nickname,path,DiscoverPage.this.nickname)==false) {
...
//logging error
	                		error.log(String.format("%s again liked %s's photo twice ",DiscoverPage.this.nickname,nickname));
            ...
            dislikeButton.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    try {
                    	if (hasUserDislikedImage(nickname,path,DiscoverPage.this.nickname)==false && hasUserLikedImage(nickname,path,DiscoverPage.this.nickname)==false) {
	                    	int currentLikes = Integer.parseInt(likeButton.getText().split(" ")[1]);
							int currentDislikes = Integer.parseInt(dislikeButton.getText().split(" ")[1]);
	                        currentDislikes+=1;

//logging info
	                		info.log(String.format("%s disliked %s's photo ",DiscoverPage.this.nickname,nickname));
	                        ...
	                        writeDislikedInfo(nickname, path, DiscoverPage.this.nickname);
	                        deleteLikeInfo(nickname, path,DiscoverPage.this.nickname);
                    	}
                    	else if (hasUserDislikedImage(nickname,path,DiscoverPage.this.nickname)==false) {
	                    	int currentLikes = Integer.parseInt(likeButton.getText().split(" ")[1]);
							int currentDislikes = Integer.parseInt(dislikeButton.getText().split(" ")[1]);
	                        currentDislikes+=1;
                        	currentLikes-=1;
...
//logging error
	                		error.log(String.format("%s again disliked %s's photo twice ",DiscoverPage.this.nickname,nickname));
            ...
            noReaction.addActionListener(
            		new ActionListener() {
            			public void actionPerformed(ActionEvent a) {
            				...
            				if (hasUserDislikedImage(nickname, path, DiscoverPage.this.nickname)==true) {
 ...
 //logging info
    	                		info.log(String.format("%s removed his/her dislike from %s's photo ",DiscoverPage.this.nickname,nickname));
            					try {
            				...
            				else if (hasUserLikedImage(nickname, path, DiscoverPage.this.nickname)==true) {
...
//logging info
    	                		info.log(String.format("%s removed his/her like from %s's photo ",DiscoverPage.this.nickname,nickname));
            					try {
...
//logging info
                		...
                		info.log(String.format("%s commented on %s's %s image ",DiscoverPage.this.nickname,nickString,nameOfImage));
		                commentsPanel.add(commentLabel);
...
public static void writeLikedInfo(String userPosted, String imagePath, String userLiked) {
	 ...
	 * This method writes to WhoLikedWhom text file indicating that which user liked which user's which photo
	 */
public static void writeDislikedInfo(String userPosted, String imagePath, String userDisliked) {
    String filePath = "/Users/Raul/IdeaProjects/COMP132-Project/COMP132_Project/DiscoverPage/WhoLikedWhom.txt";

    try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath, true))) {
        writer.write(userPosted + " " + imagePath + " "+ userDisliked +" Disliked\n");
        System.out.println("Information written to file successfully!");
    } catch (IOException e) {
        e.printStackTrace();
    }
